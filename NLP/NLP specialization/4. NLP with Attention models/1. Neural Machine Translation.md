...
Recurrent models typically take in a sequence in the order it is written and use that to output a sequence. Each element in the sequence is associated with its step in computation time ttt. (i.e. if a word is in the third element, it will be computed at t3t3​t, start subscript, 3, end subscript). These models generate a sequence of hidden states htht​h, start subscript, t, end subscript, as a function of the previous hidden state ht−1ht−1​h, start subscript, t, −, 1, end subscript and the input for position t.
