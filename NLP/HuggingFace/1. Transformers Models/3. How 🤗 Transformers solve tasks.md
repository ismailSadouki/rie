...
1. The input embeddings are passed through multiple encoder layers to output some final hidden states.
...
> [!note] Notice the parallel between ViT and BERT: both use a special token (`[CLS]`) to capture the overall representation, both add position information to their embeddings, and both use a Transformer encoder to process the sequence of tokens/patches.
