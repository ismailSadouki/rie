...
![](https://i.imgur.com/oPqpRCh.png)
Using those attention matrices with less parameters then allows the model to have inputs having a bigger sequence length.

Longformer
- Uses a **sliding window** attention: each token only attends to nearby tokens (local context).
    
- Adds **global attention tokens** for key positions (e.g., CLS token, headings in documents).
    
- Complexity: **O(n)** (linear in sequence length).
    

👉 Good for **long documents** (e.g., scientific papers, legal contracts).

### Axial positional encodings
![](https://i.imgur.com/u7MbbpB.png)
Reformer (2020)

- Uses **locality-sensitive hashing (LSH)** to approximate nearest neighbors in attention.
    
- Instead of attending to _all_ tokens, each token only attends to tokens in the **same hash bucket** (likely similar).
    
- Also introduces **reversible layers** (saves memory).
    
- Complexity: **O(n log n)**.
    

👉 Useful for **very long sequences** like genomics or long time-series.


## 📊 Trade-offs

- **Full attention** = best accuracy, worst scalability.
    
- **Sparse / efficient attention** = scalable, sometimes a small accuracy drop, but crucial for _long-context tasks_.
