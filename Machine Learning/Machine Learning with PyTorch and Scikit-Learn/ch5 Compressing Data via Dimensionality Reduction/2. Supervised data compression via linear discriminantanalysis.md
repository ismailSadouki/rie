...
> [!note] 🎯 What is LDA really doing?
> The **goal of LDA** is to find a **projection** (a new feature space) where:
> 1. **Samples from the same class** are **close together**.
> 2. **Samples from different classes** are **far apart**.
> ### 🔬 How does it do that?
> LDA solves an optimization problem:
> - **Maximize:**  
The **between-class variance**: how far apart the class means are.
> - **Minimize:**
> The **within-class variance**: how spread out the points are **within each class**.
> This gives you a **linear combination of features** (i.e., a set of projection axes) that are best at **discriminating between classes**.
> 
> `ChatGPT`
# PCA Vs LDA
...
<mark>A.M. Martinez reported that preprocessing via PCA tends to result in better classification results in an image recognition task in certain cases, for instance, if each class consists of only a small number of examples </mark>
