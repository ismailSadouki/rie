...
In order to find the optimal weights of the model, we optimized an objective function that we defined as the mean of squared errors (MSE) loss function L(w). Furthermore, we multiplied the gradient by a factor, the learning rate 𝜂, which we had to choose carefully to balance the speed of learning against the risk of overshooting the global minimum of the loss function.
